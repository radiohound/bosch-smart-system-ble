...
| [`docs/ISSUE-LOG.md`](docs/ISSUE-LOG.md) | **The bike's issue log — and three ways to read it wrong** — how to query active issues over USB (`EXECUTE_INFORMATION_MANAGER_COMMAND_*`, dealer tiers answer USB while `_RIDER` answers BLE), the record's wire format, and three traps that yield plausible wrong answers rather than errors: the list is 0-indexed, zigzag doubles every timestamp past 2³¹ (a date near 1947 is that bug), and catalog codes are hex strings. Includes whether connecting a diagnostic tool leaves a trace — on this bike, no |
| [`data/ble_accessibility.csv`](data/ble_accessibility.csv) | Every registry address × its result on **each transport and verb**, keyed to Remko's `MCSP` address: BLE (`value` / `supported-empty` / `not-available` / `command` / `passive`), USB read (`usb_2026_08_21`), and USB subscribe (`usb_subscribe_2026_08_21` — all 417 read-`DENIED` addresses refuse SUBSCRIBE too, so `DENIED` is policy on the address, not a verb artifact) |
